...
export default function App(props: Props): VNode {
  ...
  useLayoutEffect(() => {
    ...
    const draw = (ctx: CanvasRenderingContext2D): void => {
      ...
      const powerX = Math.ceil(
        Math.log10(minimumcnumSpacing * scaleRef.current[0])
      )
      const powerY = Math.ceil(
        Math.log10(minimumcnumSpacing * scaleRef.current[1])
      )
      ...
      const xIndexOffset = Math.ceil(
        (translateRef.current[0] * scaleRef.current[0]) / factorX
      )
      const yIndexOffset = Math.ceil(
        (translateRef.current[1] * scaleRef.current[1]) / factorY
      )
      const firstXValue = xIndexOffset * factorX
      const firstYValue = yIndexOffset * factorY
      const firstXPosition =
        firstXValue / scaleRef.current[0] - translateRef.current[0]
      const firstYPosition =
        translateRef.current[1] - firstYValue / scaleRef.current[1]
      ...
      for (let i = 0; i <= yLineCount; i++) {
        ...
        if (i === yMiddleLineIndex) {
          for (let j = 0; j <= xLineCount; j++) {
            ...
            // rotate, to avoid overlap with long labels and small cnum
            ctx.save()
            ctx.translate(
              x + xLabelOffset[0] - label.length * 4,
              y + xLabelOffset[1] + label.length * 3
            )
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent): void => {
      if (
        typeof lastMousePosition[0] === 'undefined' ||
        typeof lastMousePosition[1] === 'undefined'
      )
        return
      ...
      if (mouseDown & 4) {
        const zoomTo = [
          (lastMousePosition[0] + translateRef.current[0]) *
            scaleRef.current[0],
          (heightRef.current - lastMousePosition[1] + translateRef.current[1]) *
            scaleRef.current[1],
        ]
        if (
          typeof zoomTo[0] === 'undefined' ||
          typeof zoomTo[1] === 'undefined'
        )
          return
        ...
        if (
          typeof zoomLastPosition[0] === 'undefined' ||
          typeof zoomLastPosition[1] === 'undefined'
        )
          return
        ...
        if (
          typeof zoomToPosition[0] === 'undefined' ||
          typeof zoomToPosition[1] === 'undefined'
        )
          return
    ...
    const handleTouchMove = (event: TouchEvent): void => {
      if (
        typeof lastTouch1Position[0] === 'undefined' ||
        typeof lastTouch1Position[1] === 'undefined'
      )
        return
      if (
        typeof lastTouch2Position[0] === 'undefined' ||
        typeof lastTouch2Position[1] === 'undefined'
      )
        return
      if (
        typeof event.touches[0] === 'undefined' ||
        typeof event.touches[1] === 'undefined'
      )
        return
      if (lastTouch1Position[0] > -1) {
        if (event.touches.length === 1) {
          ...
          if (lastTouch2Position[0] > -1) {
            ...
            if (
              typeof touchMidpoint[0] === 'undefined' ||
              typeof touchMidpoint[1] === 'undefined'
            )
              return
            const zoomTo = [
              (touchMidpoint[0] + translateRef.current[0]) *
                scaleRef.current[0],
              (heightRef.current - touchMidpoint[1] + translateRef.current[1]) *
                scaleRef.current[1],
            ]
            if (
              typeof zoomTo[0] === 'undefined' ||
    ...
    const handleWheel = (event: WheelEvent): void => {
      ...
      if (
        ...
        return
      if (
        typeof translateRef.current[0] === 'undefined' ||
        typeof translateRef.current[1] === 'undefined'
      )
        return
      if (
        typeof scaleRef.current[0] === 'undefined' ||
        typeof scaleRef.current[1] === 'undefined'
      )
        return
      const zoomTo = [
      ...
      if (typeof zoomTo[0] === 'undefined' || typeof zoomTo[1] === 'undefined')
        return
      const zoomLastPosition = [
        zoomTo[0] / scaleRef.current[0] - translateRef.current[0],
        zoomTo[1] / scaleRef.current[1] - translateRef.current[1],
    ...
    const handleKeyDown = (event: KeyboardEvent): void => {
      ...
      switch (event.code) {
        ...
        case 'NumpadDecimal':
          {
            const initialScale =
              props.initialScale ?? 8 / (centerRef.current[0] ?? 1)
            scaleRef.current[0] = scaleRef.current[1] = initialScale
            setScale(scaleRef.current[0], scaleRef.current[1])
          }
          break
